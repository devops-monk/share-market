...
function PoliticianCard({ politician, side, stockMap, onlyTracked }: {
  ...
  // Disclosure recency is what tells you whether a quiet card means stale data
  // or a member who simply has not traded.
  const lastDisclosed = useMemo(() => {
    const filed = politician.trades.map(t => t.filedDate).filter(Boolean);
    return filed.length > 0 ? filed.reduce((a, b) => (a > b ? a : b)) : null;
  }, [politician.trades]);
    ...
    <div className="card p-4 flex flex-col">
      ...
      <div className="mt-3 space-y-1.5 flex-1">
        {visible.length === 0 ? (
          ...
          <div key={`${t.ticker}-${t.tradedDate}-${i}`} className="flex items-center gap-2 text-xs">
            ...
            <span
              className="w-20 text-right flex-shrink-0 whitespace-nowrap leading-tight"
              title={`Traded ${formatDate(t.tradedDate)}${t.filedDate ? ` · disclosed ${formatDate(t.filedDate)}` : ''}`}
            >
              <span className="block t-faint">{formatShortDate(t.tradedDate)}</span>
              {t.filedDate && (
                <span className="block text-[10px] t-faint opacity-70">
                  disclosed {formatShortDate(t.filedDate)}
                </span>
              )}
            </span>
          </div>
      ...
      <div className="flex items-center justify-between gap-2 mt-3 pt-2.5 border-t border-surface-border">
        ...
        <span
          ...
          title="When this member last traded, and when that trade became public"
        >
          {politician.lastTraded ? `Last trade ${daysAgo(politician.lastTraded)}d ago` : ''}
          {lastDisclosed && ` · disclosed ${daysAgo(lastDisclosed)}d ago`}
        </span>
      </div>
    </div>
  );
